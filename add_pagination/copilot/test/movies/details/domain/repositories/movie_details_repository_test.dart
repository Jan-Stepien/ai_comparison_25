...
void main() {
  ...
  group('MovieDetailsRepository', () {
    test(
      'fetchMovieDetails should fetch from remote and save to local',
      () async {
        when(
          () => mockRemoteService.fetchMovieDetails(1),
        ).thenAnswer((_) async => mockMovieDetails);
        when(
          () => mockLocalService.saveMovieDetails(mockMovieDetails),
        ).thenAnswer((_) async {});
    ...
    test('stream should return local service stream', () {
      when(() => mockLocalService.stream).thenAnswer(
        (_) => Stream.fromIterable([mockMovieDetails, null, mockMovieDetails]),
      );
