...
import 'package:copilot/core/extensions/build_context_extension.dart';
import 'package:copilot/movies/details/domain/repositories/movie_details_repository.dart';
import 'package:copilot/movies/details/presentation/state/movie_details_bloc/movie_details_bloc.dart';
import 'package:copilot/movies/details/presentation/widgets/movie_detail_item.dart';
...
class MovieDetailsView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final status = context.select(
      (MovieDetailsBloc bloc) => bloc.state.loadingStatus,
    );
    final details = context.select(
      (MovieDetailsBloc bloc) => bloc.state.details,
    );
    final shouldWatch = context.select(
      (MovieDetailsBloc bloc) => bloc.state.shouldWatch,
    );
    ...
    return Scaffold(
      ...
      body: switch (status) {
        ...
        LoadingStatus.error => Center(
          child: Text(
            context.select(
              (MovieDetailsBloc bloc) => bloc.state.error ?? context.l10n.error,
            ),
          ),
        ),
